...
from backend.orchestrator.events import Event, bus
...
# How long one stream may stay open before the server ends it and lets the
# client reconnect. An SSE response only finishes when its generator
# returns, and uvicorn's graceful shutdown waits for in-flight responses
# *before* running lifespan shutdown -- so a stream that never ends holds
# the whole server in "Waiting for connections to close" forever. Under
# `--reload` that means the listening socket is already closed while no
# replacement worker can start, and every subsequent request hangs until
# the browser gives up. Capping the lifetime makes that self-healing, and
# costs nothing: the client reconnects with `Last-Event-ID` and item 2 of
# the contract above replays whatever it missed.
