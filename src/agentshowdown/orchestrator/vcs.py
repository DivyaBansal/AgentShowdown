...
def fetch_and_push_branch(repo_path: str, sandbox_name: str, branch: str) -> None:
    """Fetches a sandbox's branch and pushes it to origin in one step."""
    fetch_branch(repo_path, sandbox_name)
    push_branch(repo_path, sandbox_name, branch)
...
def run_in_worktree(repo_path: str, ref: str, command: str) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory(prefix="agentshowdown-verify-") as tmp:
        ...
        try:
            return run(["bash", "-c", f"cd {shlex.quote(worktree)} && {command}"], check=False)
        finally:
